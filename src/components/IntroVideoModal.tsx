...
export const IntroVideoModal: React.FC<IntroVideoModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/95 backdrop-blur-md transition-all duration-700 animate-fade-in font-sans p-4">
      ...
      <div className={`relative w-full bg-slate-900 border border-slate-800/80 rounded-3xl shadow-2xl overflow-hidden flex flex-col items-center transition-all duration-500 ${
        isShorts ? 'max-w-[380px] sm:max-w-[400px] shadow-indigo-500/10' : 'max-w-4xl'
      }`}>
        {/* Top bar with tiny badge and close button */}
        <div className="w-full flex items-center justify-between px-5 py-3.5 border-b border-slate-800/60 bg-slate-950/40 relative z-10">
          <div className="flex items-center gap-2 overflow-hidden mr-2">
            ...
            <span className="text-[10px] sm:text-xs font-black uppercase text-slate-300 tracking-wider truncate">
              {badgeText || 'Video de Presentación Oficial de la Campaña 🇻🇪'}
            </span>
          </div>
          <button
            onClick={handleClose}
            className="p-1.5 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded-xl transition cursor-pointer shrink-0"
        ...
        <div className={`w-full bg-black relative border-b border-slate-800/60 shadow-inner overflow-hidden ${
        ...
        <div className="w-full p-5 bg-slate-950/60 flex flex-col items-stretch gap-3">
          ...
          <button
            onClick={handleClose}
            className="w-full px-5 py-3 bg-gradient-to-r from-[#008CBA] to-indigo-600 hover:from-[#007095] hover:to-indigo-700 text-white font-black text-xs uppercase tracking-wider rounded-xl transition-all shadow-xl shadow-indigo-500/10 hover:shadow-indigo-500/20 active:scale-95 cursor-pointer flex items-center justify-center gap-2"
          >
