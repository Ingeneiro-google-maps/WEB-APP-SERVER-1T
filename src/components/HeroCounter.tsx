    ...
    <section id="iniciativa" className="bg-[#F4F7F9] text-[#1A202C] pt-8 pb-12 px-4 sm:px-6 lg:px-8 border-b border-slate-200">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-stretch bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
          ...
          {/* Left Column: Bold Typography Hero Headline */}
          <div className="lg:col-span-7 p-8 sm:p-12 flex flex-col justify-between lg:border-r border-slate-200 bg-gradient-to-br from-white to-slate-50">
            <div>
              <span className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-[#008CBA] text-white text-[11px] font-black tracking-[0.2em] uppercase rounded mb-6 shadow-sm">
                <span>{state.heroBadgeText || '🇻🇪 Emergencia Humanitaria • Terremoto Venezuela'}</span>
              </span>
              
              <h1 className="text-4xl sm:text-6xl lg:text-[76px] leading-[0.88] font-black tracking-tighter uppercase mb-6 text-[#1A202C]">
                {state.heroTitleRow1 || 'AYUDA VITAL'} <span className="text-3xl sm:text-5xl align-middle">🇻🇪</span> <br />
                <span className="text-[#008CBA]">{state.heroTitleRow2 || 'POR 1 TONELADA'}</span> <br />
                {state.heroTitleRow3 || 'DESDE ESPAÑA'}
              </h1>
              
              <p className="text-base sm:text-xl text-slate-500 font-medium leading-relaxed max-w-xl mb-8">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 pt-6 border-t border-slate-200/80">
              <div className="flex items-center gap-3 px-4 py-2.5 bg-slate-100 rounded-xl border border-slate-200">
                <span className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-ping"></span>
                <span className="text-xs font-bold text-slate-600 uppercase tracking-wider italic">
              ...
              <button
                ...
                disabled={syncing}
                className="text-xs font-black uppercase text-[#008CBA] hover:text-[#006080] inline-flex items-center gap-1.5 underline decoration-2 underline-offset-4 cursor-pointer"
              >
